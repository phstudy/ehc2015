...
public class LineReaderV3 implements ILineReader {
    ...
    private OrderPlistFinder orderPlistFinder;
    ...
    private int readDefaultLine(Text str, int maxLineLength, int maxBytesToConsume) throws IOException {
        ...
        int countDown = 100;
        while (!orderPlistFinder.isEOF && countDown-- > 0) {
            orderPlistFinder.sink();
        }

        int size = orderPlistFinder.flushResult(bufferForOrderPlist);
        str.clear();
        if(size <=0){
            return 0;
        }
